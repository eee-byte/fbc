...
#    fbchaind start --pruning=nothing --rpc.unsafe \
    fbchaind start --rpc.unsafe \
      ...
      --rest.laddr "tcp://localhost:8545" > fbc.txt 2>&1 &
...
fbchaincli config output json
fbchaincli config indent true
fbchaincli config trust-node true
fbchaincli config keyring-backend test
...
# fbchaincli tx send captain 0x83D83497431C2D3FEab296a9fba4e5FaDD2f7eD0 1okt --fees 1okt -b block -y
